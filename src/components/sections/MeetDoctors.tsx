      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="max-w-3xl mb-16">
          ...
          <h2 className="text-4xl sm:text-5xl font-bold tracking-tight">
            Our Ivy League <span className="font-serif italic font-normal text-teal-300">Faculty Specialists</span>
          </h2>
        </div>

        {/* Magazine Spotlight Split */}
        <div className="rounded-3xl p-8 lg:p-12 bg-slate-950/80 border border-slate-800 grid lg:grid-cols-12 gap-12 items-center shadow-2xl">
          
          {/* Left Portrait Frame (5 cols) */}
          <div className="lg:col-span-5 relative rounded-2xl overflow-hidden aspect-[4/5] bg-slate-900 border border-slate-800">
            <img 
            ...
            />
            <div className="absolute top-4 left-4 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/90 border border-slate-800 text-teal-300 text-xs font-bold shadow-sm">
              <Sparkles className="w-3.5 h-3.5" /> {currentDoc.tag}
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div>
              ...
              <h3 className="text-3xl font-bold text-white">{currentDoc.name}</h3>
            </div>
            ...
            <div className="flex flex-wrap gap-3 text-xs font-medium text-slate-300">
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800">
                <GraduationCap className="w-4 h-4 text-teal-400" /> {currentDoc.education}
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800">
                <Award className="w-4 h-4 text-amber-400" /> {currentDoc.experience}
              </span>
            </div>

            <blockquote className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 text-sm italic text-slate-300 font-serif leading-relaxed">
              "{currentDoc.quote}"
            </blockquote>
            ...
            <div className="space-y-2">
              {currentDoc.highlights.map((h, idx) => (
                <div key={idx} className="flex items-center gap-2 text-xs font-medium text-slate-300">
                  <CheckCircle2 className="w-4 h-4 text-teal-400 shrink-0" /> {h}
                </div>
              ))}
            </div>
            ...
            <div className="pt-4">
              <Link to="/contact">
                <Button className="rounded-full bg-gradient-to-r from-sky-500 to-teal-500 text-white px-6 font-semibold shadow-md">
